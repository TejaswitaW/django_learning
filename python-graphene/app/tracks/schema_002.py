...
class Query(graphene.ObjectType):
    ...
    def resolve_tracks(self, info, search=None):
        if search:
            # to filter method, we have to provide what field we want
            # the search text to match on and also what type of match we
            # want to be performed
            # here example we want our search text to match and filter our tracks
            # according to just the title
            # we want to match it according to whether that search text provided
            # starts with or the title starts with the provided search text
